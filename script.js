  ...
  if(fleet&&fleetItems.length){
    ...
    fleetItems.forEach((item,i)=>{
      ...
      if(isMobile){
        ...
        const startAt=i/(n+0.8)*0.86;
        const span=.14;
        const t=clamp((fp-startAt)/span,0,1);
        const eased=t*t*(3-2*t);
        item.style.opacity=String(eased);
        item.style.visibility=eased>.001?'visible':'hidden';
        const slide=dir*(1-eased)*118;
        const rise=(1-eased)*7;
        item.style.transform=`translate3d(${slide}vw,${rise}vh,0)`;
        if(img) img.style.transform=`scale(${0.98+eased*0.06})`;
      }
    });
    if(fleetFill)fleetFill.style.width=`${fp*100}%`;
    if(fleetCounter){
      const active=Math.min(Math.floor(fp*n)+1,n);
      fleetCounter.textContent=String(active).padStart(2,'0');
    }
  }
  if(experience&&car){
