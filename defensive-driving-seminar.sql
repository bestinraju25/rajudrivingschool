...
create unique index if not exists seminar_event_email_unique on public.seminar_registrations(event_id,lower(email)) where status <> 'cancelled' and nullif(trim(email),'') is not null;
...
begin
  ...
  select * into existing from public.seminar_registrations
   where event_id=p_event_id and status <> 'cancelled'
     and (phone=regexp_replace(p_phone,'[^0-9]','','g') or (nullif(trim(coalesce(p_email,'')),'') is not null and lower(email)=lower(trim(p_email))))
   limit 1;
