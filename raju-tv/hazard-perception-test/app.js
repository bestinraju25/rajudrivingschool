...
let clips=[], order=[], pos=0, responses=[], scores=[], running=false, finishing=false, timer=null, clipClock=null, responseLocked=false;
const TV_MODE=(()=>{const u=navigator.userAgent||'';return /smart-tv|smarttv|hbbtv|tizen|web0s|webos|netcast|viera|bravia|googletv|android tv|aftb|aftt|appletv|tv safari/i.test(u)||window.matchMedia?.('(display-mode: fullscreen)')?.matches&&/tv|tizen|webos|hbbtv/i.test(u)})();
if(TV_MODE)document.body.classList.add('tv-mode');
const localMp4=code=>`./videos/mp4/${Number(code)}.mp4`;
const isBundledCode=code=>Number(code)>=1&&Number(code)<=10;
function stopClipClock(){if(clipClock){clearInterval(clipClock);clipClock=null}if(timer){clearTimeout(timer);timer=null}}
function startClipClock(){stopClipClock();clipClock=setInterval(()=>{if(running&&!finishing&&Number(video.currentTime)>=Number(C.clipLimitSeconds)){finishClip()}},100)}
...
async function playExamVideo(){
  video.controls=false;
  video.removeAttribute('controls');
  video.muted=TV_MODE?true:!sound;
  try{await video.play();$('playOverlay').hidden=true;running=true;startClipClock();return true}catch(e){
    if(TV_MODE){
      video.muted=true;
      try{await new Promise(r=>setTimeout(r,120));await video.play();$('playOverlay').hidden=true;running=true;startClipClock();return true}catch{}
    }
    running=false;
...
function loadClip(){
  resetClip();running=false;stopClipClock();video.pause();video.removeAttribute('src');
  const item=order[pos];
  video.src=(TV_MODE&&isBundledCode(item.clip_code))?localMp4(item.clip_code):(item.file||item.video_path);
  video.preload='auto';video.controls=false;video.muted=TV_MODE?true:!sound;video.load();$('playOverlay').hidden=true;
  video.onloadedmetadata=async()=>{video.currentTime=0;update();timeline();const ok=await playExamVideo();if(!ok){toast('Video could not start. Please try again.',true)}};
  video.onerror=()=>{running=false;stopClipClock();$('playOverlay').hidden=true;toast('Video could not be loaded',true)};
  if(pos+1<order.length&&TV_MODE&&isBundledCode(order[pos+1].clip_code)){
    const pre=document.createElement('video');pre.preload='auto';pre.muted=true;pre.src=localMp4(order[pos+1].clip_code);pre.setAttribute('aria-hidden','true');pre.style.cssText='position:absolute;width:1px;height:1px;opacity:0;pointer-events:none;';document.body.appendChild(pre);setTimeout(()=>pre.remove(),45000);
  }
}
async function finishClip(){if(finishing)return;finishing=true;running=false;stopClipClock();video.pause();scores.push(Math.max(0,Math.min(10,scoreThis())));pos++;if(pos>=order.length){await finishExam();return}setTimeout(()=>{finishing=false;loadClip()},220)}
...
video.setAttribute('controlsList','nodownload noplaybackrate nofullscreen');video.disablePictureInPicture=true;video.disableRemotePlayback=true;video.addEventListener('contextmenu',e=>e.preventDefault());
video.addEventListener('pause',()=>{if(TV_MODE&&running&&!finishing){setTimeout(()=>{if(TV_MODE&&running&&!finishing&&video.paused)playExamVideo()},120)}});
video.addEventListener('waiting',()=>{if(TV_MODE)video.classList.add('buffering')});video.addEventListener('canplay',()=>video.classList.remove('buffering'));
$('resultClose').onclick=()=>{show('resultScreen',false);show('startScreen',true);};$('newBtn').onclick=()=>{show('resultScreen',false);show('startScreen',true);};$('printCertBtn').onclick=()=>printCertificate();const viewAttemptsBtn=$('viewAttemptsBtn');if(viewAttemptsBtn)viewAttemptsBtn.onclick=openAttempts;$('attemptSearchBtn').onclick=lookupAttempts;$('attemptCloseBtn').onclick=closeAttempts;$('attemptPhone').onkeydown=e=>{if(e.key==='Enter')lookupAttempts()};$('attemptsModal').onclick=e=>{if(e.target===$('attemptsModal'))closeAttempts()};$('skipBtn').onclick=finishClip;$('soundBtn').onclick=()=>{if(TV_MODE)return;sound=!sound;video.muted=!sound;$('soundBtn').textContent=sound?'🔊':'🔇'};$('fullBtn').onclick=()=>{if(!document.fullscreenElement)document.documentElement.requestFullscreen?.();else document.exitFullscreen?.()};$('helpBtn').onclick=()=>toast('Tap when a developing hazard becomes apparent. You can make up to 5 responses per clip.');$('exitBtn').onclick=()=>{if(confirm('Exit the examination? This attempt will be incomplete.')){stopClipClock();video.pause();running=false;show('examScreen',false);show('startScreen',true);}};$('playNow').onclick=()=>playExamVideo().catch(()=>toast('Video could not start',true));
