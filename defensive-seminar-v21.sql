-- Raju Driving School — Defensive Driving Seminar v21 migration
-- Run AFTER the existing seminar SQL + v20 QR migration.
-- Changes: modal-friendly booking flow, optional email, one active booking per mobile,
-- mobile-only booking lookup/cancellation, and event-scoped QR check-in.

create index if not exists seminar_reg_phone_idx on public.seminar_registrations(phone);

-- Keep the registration RPC authoritative: one non-cancelled seminar booking per mobile number.
...
declare
  ...
  new_code text;
begin
  -- Serialize registrations for the same mobile number so double-taps/race conditions
  -- cannot create two active bookings for one phone.
  ...
  -- One active booking per mobile across the whole seminar programme.
  ...
  -- Pick the first free seat so a cancelled booking immediately releases its seat.
  select min(s)::integer into next_seat
  from generate_series(1,ev.capacity) as s
  where not exists (
    select 1 from public.seminar_registrations r
    where r.event_id=p_event_id and r.seat_number=s and r.status <> 'cancelled'
  );
  ...
  return jsonb_build_object(
    'id',new_id,'booking_code',new_code,'full_name',trim(p_full_name),'phone',normalized_phone,
    'seat_number',next_seat,'event_date',ev.event_date,'start_time',ev.start_time,
    'end_time',ev.end_time,'venue',ev.venue,'title',ev.title
  );
end;
$$;
