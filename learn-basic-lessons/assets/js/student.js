...
import { posterImageUrl } from './poster-assets.js';
...
function card(s){
  const m=state.mediaMap[s.sign_id]; const photo=publicMediaUrl(m?.photo_path) || posterImageUrl(s.sign_id);
...
function select(id){
 ...
 const photo=publicMediaUrl(m?.photo_path) || posterImageUrl(s.sign_id); if(photo){$('detail-photo').src=photo;$('detail-photo').classList.remove('hide');$('detail-visual').classList.add('hide')}else{$('detail-photo').classList.add('hide');$('detail-visual').classList.remove('hide');$('detail-visual').innerHTML=visualMarkup(s)}
